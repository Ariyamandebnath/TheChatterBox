...
declare global {
    namespace Express {
        interface Request {
            user?:typeof User; // Add the user property with the type of your User model
        }
    }
}

export const verifyJWT = asyncHandler(async (req: express.Request, res, next) => {
  const token =
  ...
  try {
    const decodedToken = jwt.verify(token, process.env.ACCESS_TOKEN_SECRET as string) as JwtPayload; const user = await User.findById(decodedToken?._id).select(
